...
describe("InstituteService", () => {
    ...
    describe("createInstitute", () => {
        ...
        it("should throw BadRequestException if domain is already in use", async () => {
            mockPrismaService.institute.findUnique.mockResolvedValue({ id: "existing" });

            await expect(service.createInstitute(createData)).rejects.toThrow(BadRequestException);
        });
        ...
        it("should successfully create an institute with roles and admin user", async () => {
            ...
            const mockInstitute = { id: "new-inst-id", name: createData.name, domain: createData.domain };
            ...
            mockTx.role.create.mockResolvedValueOnce(mockAdminRole).mockResolvedValueOnce({ id: "p-role-id", name: UserRole.PRINCIPAL });
