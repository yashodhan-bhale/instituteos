...
export class StudentFormat1Strategy extends BaseImportStrategy<Prisma.StudentCreateManyInput> {
    ...
    private readonly columnMap: Record<string, keyof Prisma.StudentCreateManyInput> = {
        'GR Set': 'grSet',
        'GR No.': 'grNo',
        'Division': 'division',
    ...
    parseAndValidate(
        ...
        if (headerIndex === -1) {
            errors.push({ row: 0, errors: ["Could not find 'Student Name' header row."] });
        ...
        dataRows.forEach((row, index) => {
            ...
            if (rowErrors.length > 0) {
                ...
                entities.push({
                    ...
                    studentIdTag: rowData.studentIdTag ? String(rowData.studentIdTag).trim() : null,
                    motherName: rowData.motherName ? String(rowData.motherName).trim() : null,
